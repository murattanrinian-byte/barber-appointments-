...
def next_id(k): idc[k]+=1; return idc[k]
def parse_hhmm(s): h,m=[int(x) for x in s.split(":")]; return time(h,m)
def local_date_bounds(date_str):
...
class ServiceCreate(BaseModel):
    name:str; duration_min:int=Field(gt=0); price:int=Field(ge=0)
class ServiceOut(BaseModel):
    id:int; name:str; duration_min:int; price:int
class BarberCreate(BaseModel):
    full_name:str; color_hex:Optional[str]=None
class BarberOut(BaseModel):
    id:int; full_name:str; color_hex:str
class WorkingHourCreate(BaseModel):
    barber_id:int; weekday:int=Field(ge=1,le=7); start_time:str; end_time:str
class WorkingHourOut(BaseModel):
    id:int; barber_id:int; weekday:int; start_time:str; end_time:str
class CustomerCreate(BaseModel):
    full_name:str; phone:str
class CustomerOut(BaseModel):
    id:int; full_name:str; phone:str
class AppointmentCreate(BaseModel):
    shop_id:int=1; barber_id:int; customer_id:int; service_id:int
    starts_at:datetime; duration_override_min:Optional[int]=None
class AppointmentOut(BaseModel):
    id:int; barber_id:int; customer_id:int; service_id:int
    starts_at:datetime; ends_at:datetime; status:str
...
def create_service(p:ServiceCreate):
    ...
    services.append(s); return s

@app.get("/services",response_model=List[ServiceOut])
def list_services(): return services
...
def create_barber(p:BarberCreate):
    ...
    barbers.append(b); return b

@app.get("/barbers",response_model=List[BarberOut])
def list_barbers(): return barbers

@app.post("/working-hours",response_model=WorkingHourOut,status_code=201)
def create_wh(p:WorkingHourCreate):
    wh={"id":next_id("wh"),"barber_id":p.barber_id,"weekday":p.weekday,"start_time":p.start_time,"end_time":p.end_time}
    working_hours.append(wh); return wh
...
def get_slots(barber_id:int,date:str=Query(...),service_id:int=Query(...)):
    ...
    slots=[]
    for w in whs:
        st=parse_hhmm(w["start_time"]); en=parse_hhmm(w["end_time"])
        ...
        while t+duration<=we:
            end_t=t+duration
            conflict=any((t<a["ends_at"] and end_t>a["starts_at"]) for a in appointments if a["barber_id"]==barber_id)
            if not conflict: slots.append(t.isoformat())
            t+=duration
    return {"slots":slots}
...
def create_appt(p:AppointmentCreate):
    ...
    if not svc: raise HTTPException(400,"Invalid service")
    ...
    a={"id":next_id("appt"),"shop_id":p.shop_id,"barber_id":p.barber_id,"customer_id":p.customer_id,
       "service_id":p.service_id,"starts_at":p.starts_at,"ends_at":ends,"status":"confirmed"}
    appointments.append(a); return a

@app.get("/appointments",response_model=List[AppointmentOut])
def list_appts(date:Optional[str]=None,barber_id:Optional[int]=None):
    out=appointments[:]
    if barber_id: out=[a for a in out if a["barber_id"]==barber_id]
    if date: 
        s,e=local_date_bounds(date); out=[a for a in out if a["starts_at"]>=s and a["starts_at"]<e]
