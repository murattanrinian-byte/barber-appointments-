...
barbers = []         # {id,full_name,color_hex}
working_hours = []   # {id,barber_id,weekday,start_time,end_time}
customers = []       # {id,full_name,phone}
appointments = []    # {id,shop_id,barber_id,customer_id,service_id,starts_at,ends_at,status}
...
def local_date_bounds(date_str: str):
    # "YYYY-MM-DD" → günün başı/sonu (yerel TZ)
...
class WorkingHourCreate(BaseModel):
    barber_id: int
    weekday: int = Field(ge=1, le=7)  # 1 = Pazartesi
...
def create_barber(p: BarberCreate):
    b = {
        ...
        "color_hex": p.color_hex or "#777777"
    }
    barbers.append(b)
    return b
